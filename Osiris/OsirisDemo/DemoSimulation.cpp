...
void DemoSimulation::init()
{
	...
	camera()->setPosition(OVector3(-1.0f, 1.50f, -1.5f));
	camera()->setOrientation(OVector3(-45.0f, -45.0f, 0.0f));
	...
	cube->addVertexColorData(0.5f, 1.0f, 0.5f, 1.0f);
	cube->addVertexColorData(0.5f, 1.0f, 0.5f, 1.0f);
	...
	OEntity<void> *movingPiece = new OEntity<void>(NULL, NULL, torus);
	tableAtt->addPiece(movingPiece);
	entities()->add(movingPiece);
	movingPiece->state()->curr()->setMotionComponent(1, OVector3(0.3f, 0.0f, 0.3f), OState::Object);
	movingPiece->state()->curr()->scale() = OVector3(0.25);
